...
export async function getRoomWithPersonas(roomId, currentUserId) {
    try {
        ...
        // Determine sender and target personas
        const senderPersona = room.profile_1.id === currentUserId 
            ? room.profile_1.persona_type 
            : room.profile_2.persona_type;
            
        const targetPersona = room.profile_1.id === currentUserId 
            ? room.profile_2.persona_type 
            : room.profile_1.persona_type;
        ...
        console.error('Error fetching room with personas:', error);
        return null;
    }
}
...
export async function sendMessage(roomId, originalText, sender) {
    try {
        // Part A: Insert message immediately with only original_text
        const { data, error } = await supabase
            .from('messages')
            .insert({ 
                room_id: roomId, 
                original_text: originalText,
                translated_text: null, // Leave empty - will be filled by AI
                sender: sender 
            })
            .select();
        ...
        // Part B: Fire-and-forget translation call (non-blocking)
        // SECURITY: Only send messageId - server fetches everything else from DB
        if (data && data.length > 0) {
            const messageId = data[0].id;
            
            // Get auth token to send to API route
            ...
            fetch('/api/translate', {
                ...
                body: JSON.stringify({ messageId }) // Only send ID!
            }).catch(err => {
